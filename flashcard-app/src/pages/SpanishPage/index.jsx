...
export default function SpanishPage({ testing=false, token, setToken }) {
    ...
    useEffect(() => {
        async function fetchData() {
            const response = await fetch(`https://crammer-backend.onrender.com/spanish/${index}`);
            const data = await response.json();

            const response2 = await fetch(`https://crammer-backend.onrender.com/spanish`);
